...
def register_all_modules(reg: ModuleRegistry) -> None:
    """
    Register all available modules.
    This is the single source of truth for module registration.
    Called from main.py and run_telegram.py.
    """
    from app.modules.assistant.module import AssistantModule
    from app.modules.meeting.module import MeetingModule
    from app.modules.task.module import TaskModule
    ...
    reg.register(AssistantModule(None))
    reg.register(MeetingModule(None))
    reg.register(TaskModule(None))
    reg.register(FinanceModule(None))
    reg.register(ContactsModule(None))
    reg.register(IdeasModule(None))
    reg.register(BirthdayModule(None))
    reg.register(ContractModule(None))
    reg.register(ReportModule(None))
    reg.register(DebtorModule(None))
    reg.register(WhatsAppModule(None))
