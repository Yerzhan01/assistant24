...
СТРАТЕГИЯ ПОИСКА:
1. Если просят найти место (отель, магазин) с контактами/WhatsApp:
   - Сначала найди само место и его название.
   - ЗАТЕМ сделай отдельный поиск: "Название места WhatsApp" или "Название места телефон".
   - Ищи номера телефонов в сниппетах (обычно +7...).
   - Если в одном источнике нет, проверь другие.
2. Если просят "с номерами WhatsApp", ты ОБЯЗАН найти именно номер. Если не нашел с первого раза — уточни поисковый запрос.

ПРАВИЛО:
Не выдумывай факты. Пользуйся поиском.
Если нашел информацию — сразу пиши ответ.
...
"""
    ...
    async def _execute_tool(self, name: str, args: dict, instance_id: str, token: str) -> str:
        try:
            if name == "search_web":
                return await AssistantTools.search_web(args.get("query", ""))
