...
async def get_summary(
    ...
    """Get finance summary using SQL aggregation for better performance."""
    from sqlalchemy import case
    
    now = datetime.now()
    month_start = now.date().replace(day=1)
    
    # Total aggregation using SQL
    total_stmt = select(
        func.sum(case((FinanceRecord.type == "income", FinanceRecord.amount), else_=0)).label("total_income"),
        func.sum(case((FinanceRecord.type == "expense", FinanceRecord.amount), else_=0)).label("total_expense")
    ).where(FinanceRecord.tenant_id == tenant.id)
    
    total_result = await db.execute(total_stmt)
    total_row = total_result.one()
    total_income = float(total_row.total_income or 0)
    total_expense = float(total_row.total_expense or 0)
    
    # This month aggregation using SQL
    month_stmt = select(
        func.sum(case((FinanceRecord.type == "income", FinanceRecord.amount), else_=0)).label("month_income"),
        func.sum(case((FinanceRecord.type == "expense", FinanceRecord.amount), else_=0)).label("month_expense")
    ).where(
    ...
    this_month_income = float(month_row.month_income or 0)
    this_month_expense = float(month_row.month_expense or 0)
...
async def get_reports(
    ...
    """Generate finance reports with period filtering."""
    ...
    if start_date:
        stmt = stmt.where(FinanceRecord.record_date >= start_date)
    stmt = stmt.order_by(FinanceRecord.record_date)
