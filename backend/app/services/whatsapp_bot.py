...
class WhatsAppBotService:
    ...
    async def set_settings(
        ...
        """Set account settings. ONLY if they differ from current (avoids reboot loop)."""
        # 1. Get current settings
        try:
            current_settings = await self.get_settings(instance_id, token)
        except Exception:
            current_settings = {}
        ...
        # GreenAPI returns all settings, we check only what we set
        needs_update = False
        if not current_settings:
            needs_update = True
        else:
            for key, value in payload.items():
                # Note: GreenAPI might return int 3000 vs str "3000", or bool vs str
                # We do simple loose comparison
                curr_val = current_settings.get(key)
                if str(curr_val) != str(value):
                    # Special check for empty string vs None
                    if value == "" and curr_val is None:
                        continue
                    needs_update = True
                    logger.info(f"Setting '{key}' differs: '{curr_val}' != '{value}'")
                    break
        
        if not needs_update:
            logger.info("Settings match current state. Skipping setSettings (avoids reboot).")
            return {"status": "skipped", "reason": "already_set"}

        # 4. Update if needed
        url = self._build_url(instance_id, token, "setSettings")
        async with self._get_client() as client:
            response = await client.post(url, json=payload)
            logger.warning("♻️ GreenAPI settings updated. Instance will REBOOT.")
            return response.json()
    ...
    async def process_webhook(
        ...
        async with async_session_maker() as db:
            ...
            if webhook_type == "incomingMessageReceived":
                ...
                # Shadow Mode: Just save to DB, no AI processing
                return await self._save_message_to_db(db, tenant, webhook_data, is_outgoing=True)
    ...
    async def _save_message_to_db(
        ...
        """Save message to DB without AI processing."""
        try:
            ...
            return {"status": "ok", "mode": "saved_to_db"}
    ...
    async def _handle_state_change(
        ...
        # 1. Handle STARTING state (stuck check)
        if state == "starting":
            # We can't immediately invoke reboot here because it might be normal starting.
            # But we should log it prominently.
            pass

        # 2. Sync groups when WhatsApp becomes authorized
        # REMOVED by user request: Sync only on manual button press.
        # if state == "authorized":
        #    ...
    ...
    async def sync_groups_to_db(
        ...
        try:
            ...
            for group in groups:
                ...
                # Check if group already exists (Optimized check)
