...
⚠️ ӘҢГІМЕ КОНТЕКСТІ:
Хабарлама толық болмаса ("ал бүгін?", "тағы көрсет"), алдыңғы диалогты пайдалан:
- Егер бұрын тапсырмалар сұралса, "ал бүгін?" = бүгінгі тапсырмалар
- "Біріншіні" / "Соңғысын" — алдыңғы жауаптан элемент
...
⚠️ КОНТЕКСТ РАЗГОВОРА:
Если сообщение неполное ("а сегодня?", "покажи еще", "удали последнего"), используй ПРЕДЫДУЩИЙ ДИАЛОГ чтобы понять о чем речь:
- Если до этого спрашивали задачи, "а сегодня?" = задачи на сегодня
- Если до этого спрашивали долги, "покажи еще" = еще долги
- "Первого" / "Последнего" — ссылка на элемент из предыдущего ответа
...
"""
    ...
    async def process_message(
        ...
        try:
            ...
            # 4. Classification
            if on_status: await on_status("🤔 Думаю...")
            ...
            # 5. Execution
            if on_status: await on_status("⚡ Выполняю...")
            ...
            for item in intents:
                ...
                if module and module in enabled_modules:
                    ...
                    if on_status:
                        status_map = {
                            "assistant": "🔍 Ищу информацию...",
                            "task": "📋 Работаю с задачами...",
                            "contacts": "📒 Работаю с контактами...",
                            "finance": "💰 Работаю с финансами...",
                            "meeting": "📅 Работаю с встречами...",
                        }
                        status_text = status_map.get(intent, f"⚡ {intent}...")
