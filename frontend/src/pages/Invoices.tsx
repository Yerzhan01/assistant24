...
export default function Invoices() {
    ...
    const openPaymentModal = async (invoice: Invoice) => {
        ...
        setPaymentAmount(invoice.remaining_amount || invoice.amount)
        setPaymentNote('')
    ...
    const submitPayment = async () => {
        ...
        try {
            await InvoicesApi.recordPayment(paymentInvoice.id, {
                                ...
                                >
                                    ...
                                    <div className="text-right">
                                        <p className={`font-semibold ${isPaid ? 'text-green-400' : 'text-white'}`}>
                                            {formatAmount(invoice.remaining_amount || invoice.amount, invoice.currency)}
                                        </p>
                                        {invoice.paid_amount > 0 && (
                                            <p className="text-xs text-gray-400">
                                                Оплачено: {formatAmount(invoice.paid_amount, invoice.currency)}
                                            </p>
                                        )}
                                    </div>
                ...
                <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
                    <div className="bg-gray-800 rounded-2xl p-6 w-full max-w-lg border border-gray-700">
                        ...
                        <div className="mb-4 p-4 bg-gray-700/50 rounded-xl">
                            <p className="text-gray-400 text-sm">Должник</p>
                            <p className="text-white font-medium">{paymentInvoice.debtor_name}</p>
                            <div className="flex justify-between mt-2 text-sm">
                                <span className="text-gray-400">Всего:</span>
                                <span className="text-white">{formatAmount(paymentInvoice.amount, paymentInvoice.currency)}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                                <span className="text-gray-400">Оплачено:</span>
                                <span className="text-green-400">{formatAmount(paymentInvoice.paid_amount || 0, paymentInvoice.currency)}</span>
                            </div>
                            <div className="flex justify-between text-sm font-medium">
                                <span className="text-gray-400">Осталось:</span>
                                <span className="text-white">{formatAmount(paymentInvoice.remaining_amount || paymentInvoice.amount, paymentInvoice.currency)}</span>
                            </div>
                        </div>

                        <div className="space-y-4">
                            <div>
                                <label className="text-sm text-gray-400 block mb-2">Сумма оплаты</label>
                                <input
                                    type="number"
                                    value={paymentAmount}
                                    onChange={e => setPaymentAmount(Number(e.target.value))}
                                    max={paymentInvoice.remaining_amount || paymentInvoice.amount}
                                    className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-xl
                            ...
                            <div>
                                <label className="text-sm text-gray-400 block mb-2">Примечание (опционально)</label>
                                <input
                                    type="text"
                                    value={paymentNote}
                            ...
                            <div className="mt-6 pt-4 border-t border-gray-700">
                                ...
                                <div className="space-y-2 max-h-40 overflow-y-auto">
                                    {paymentHistory.map(payment => (
                                        <div key={payment.id} className="flex justify-between text-sm p-2 bg-gray-700/50 rounded-lg">
                                            <div>
                                                <span className="text-white">{formatAmount(payment.amount, paymentInvoice.currency)}</span>
                                                {payment.note && <span className="text-gray-400 ml-2">• {payment.note}</span>}
                                            </div>
